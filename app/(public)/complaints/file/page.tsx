...
export default function FileComplaintPage() {
  ...
  const [kbTarget, setKbTarget] = useState<'title' | 'description' | null>(null);
  const titleRef = useRef<HTMLInputElement>(null);
  const descRef = useRef<HTMLTextAreaElement>(null);

  /** Insert char at cursor position of the active target field */
  const handleKbKey = useCallback((char: string) => {
    const el = kbTarget === 'title' ? titleRef.current : descRef.current;
    if (!el) return;
    const { selectionStart: ss, selectionEnd: se } = el;
    const start = ss ?? el.value.length;
    const end = se ?? el.value.length;
    const newVal = el.value.slice(0, start) + char + el.value.slice(end);
    const newPos = start + char.length;
    // Trigger React synthetic via native setter
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5 max-w-3xl mx-auto">
            ...
            <div>
              ...
              <input
                ...
                onFocus={() => { if (kbOpen) setKbTarget('title'); }}
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label htmlFor="complaint-desc" className="block text-sm font-medium text-gray-700">
                ...
                <button
                  type="button"
                  onClick={() => { setKbTarget('description'); setKbOpen((o) => kbTarget === 'description' ? !o : true); }}
                  className={`text-xs flex items-center gap-1 px-2 py-1 rounded border transition-all ${
                    kbOpen && kbTarget === 'description'
                      ? 'bg-indigo-600 text-white border-indigo-600'
                      : 'border-gray-300 text-gray-500 hover:border-indigo-400 hover:text-indigo-600'
                  }`}
                  title={t('keyboard.openIndic', { defaultValue: 'Open Indian language keyboard' })}
              ...
              <textarea
                ...
                onFocus={() => { if (kbOpen) setKbTarget('description'); }}
